...
class VocabEntry(object):
    ...
    def words2charindices(self, sents):
        ...
        c_idx = [[[self.char2id[c] for c in w] for w in s] for s in sents]
        return [[[self.start_of_word] + w + [self.end_of_word] for w in s] for s in c_idx]
        ### END YOUR CODE
    ...
    def to_input_tensor_char(self, sents: List[List[str]], device: torch.device) -> torch.Tensor:
        ...
        sents_pad = pad_sents_char(sents_char,self.char2id['<pad>'])
        sents_var = torch.tensor(sents_pad,device=device,dtype=torch.long)
